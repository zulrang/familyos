...
v1: the week calendar in `src/components/calendar/`, the left nav rail, and Google Calendar as the event store. Other rail destinations are stubs.

## 2. Architectural Decisions

**Decision: Shared kiosk, no FamilyOS accounts**
- Choice: Anyone at the display can use it. Google sign-in exists only to authorize Calendar API access.
...
**Decision: v1 is Calendar + Settings + stubs**
- Choice: 7-day family calendar and Settings (Google login + calendar picker). Lists, Tasks, Rewards, Meals, Recipes, Photos, Sleep render a “not yet implemented” screen.
- Alternatives considered: Building Lists/Tasks in parallel; inventing Rewards/Meals UI with no production screen yet.
- Rationale: Requirements limit v1 to the week calendar plus nav. Invented screens become accidental product.
...
- **Authentication:** Google OAuth for Calendar API only. No FamilyOS user/password/session for household members.
...
- Unimplemented rail items stay stubs. Do not invent a visual language for Rewards, Meals, Recipes, Photos, Sleep, or Settings beyond existing chrome.
...
- Lists and Tasks should follow the design-skill kit (`ui_kits/wall-display`) after Calendar is real. Keep shell/calendar code from depending on those feature modules.
